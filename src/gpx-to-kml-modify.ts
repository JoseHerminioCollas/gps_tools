 ...
 *   - Simplifies valid tracks using Douglas–Peucker with a float tolerance.
 ...
 *
 * Usage:
 *   npx ts-node src/gpx-to-kml-modify.ts <inputFile> <outputFolder> --tolerance 0.000225 --clean_outlier true
 */
...
function gpxToKml(obj: any, tolerance: number, cleanOutlier: boolean): string {
  ...
  for (const trk of tracks) {
    ...
    // Count total points across all segments safely
    let totalPoints = 0;
    ...
    for (const seg of segArray) {
      const pts = seg.trkpt ? (Array.isArray(seg.trkpt) ? seg.trkpt : [seg.trkpt]) : [];
      if (pts.length <= 1) continue; // skip degenerate segments
...
if (!inputFile || !outputFolder) {
  console.error("Usage: npx ts-node src/gpx-to-kml-modify.ts <inputFile> <outputFolder> --tolerance <float> --clean_outlier <bool>");
  process.exit(1);
}
